...
MODEL_PATH = "..\Model\yolo11n.pt" # Path of YOLO model
...
SHOW_CAMERA_FEED = False # flag: True if you want to see the camera feed
...
# Process frame to make it grayscale and blurred
...
def annotate_frame(model, frame_to_annotate, box, color=(0, 225, 0), fontScale=1, thickness=2):
    ...
    try:
        while True:
            ...
            if SHOW_CAMERA_FEED:
                ...
                # Stop the loop by pressing the  key
...
"""
# Codice aggiuntivo che si potrebbe aggiungere per avere nei metadati dell'immagine annotata il timestamp

from PIL import Image
import piexif
import io

# Supponiamo che `annotated_image` sia un oggetto PIL.Image
# e `timestamp` sia una stringa del tipo "2025:04:12 14:30:00"
# (formato valido per EXIF DateTime)

# Converti l'immagine in bytes per poter manipolare i metadata
img_byte_arr = io.BytesIO()
annotated_image.save(img_byte_arr, format='JPEG')
img_bytes = img_byte_arr.getvalue()

# Carica i metadata EXIF esistenti (o crea un dizionario vuoto)
exif_dict = piexif.load(img_bytes)

# Aggiungi o aggiorna il campo "DateTime" nella sezione 0th (Image metadata)
exif_dict['0th'][piexif.ImageIFD.DateTime] = timestamp.encode("utf-8")

# Se vuoi un campo completamente personalizzato tipo "DateTimePlotted",
# puoi usarlo in un campo esistente come UserComment (ExifIFD) oppure taggare in XPComment (ImageIFD)
exif_dict['0th'][piexif.ImageIFD.XPComment] = timestamp.encode('utf-16le')

# Converte in byte
exif_bytes = piexif.dump(exif_dict)

# Salva l'immagine con i nuovi metadata
annotated_image.save("output_con_metadata.jpg", "jpeg", exif=exif_bytes) 
"""